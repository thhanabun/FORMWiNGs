...
def parse_imu(line: str) -> ImuSample:
    ...
    try:
        return ImuSample(
            seq=int(fields[1]),
            timestamp_s=float(fields[2]),
            acc_x_g=float(fields[3]),
            acc_y_g=float(fields[4]),
            acc_z_g=float(fields[5]),
            gyro_x_dps=float(fields[6]),
            gyro_y_dps=float(fields[7]),
            gyro_z_dps=float(fields[8]),
        )
    except ValueError as exc:
